...
async function getProblems(searchParams: SearchParams) {
    ...
    const where: Record<string, unknown> = {
        isPublic: true,
        category: "AMCAT_PYQ", // Only AMCAT PYQs
    };
...
export default async function AMCATPYQsPage({
    searchParams,
}: {
    ...
    if (!session?.user) {
        redirect("/login?callbackUrl=/amcat-pyqs");
    }
    ...
    return (
        <div className="container">
            <div className={styles.header}>
                <h1 className={styles.title}>AMCAT PYQs</h1>
                <p className={styles.subtitle}>
                    {total} previous year AMCAT questions. Practice with real exam patterns.
                </p>
            </div>
                ...
                <div className={styles.filterGroup}>
                    ...
                    <div className={styles.filterButtons}>
                        ...
                        {Object.entries(difficultyConfig).map(([key, config]) => (
                            <Link
                                key={key}
                                href={`/amcat-pyqs?difficulty=${key.toLowerCase()}`}
            ...
            <div className={styles.problemList}>
                ...
                {problems.length === 0 ? (
                    <div className={styles.emptyState}>
                        ...
                        <Link href="/amcat-pyqs" className="btn btn-primary">
                            Clear Filters
                        </Link>
                    </div>
                ) : (
